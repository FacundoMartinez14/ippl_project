...
const FinancialDashboard: React.FC = () => {
  ...
  const [recentAbonos, setRecentAbonos] = useState<{ name: string; amount: number; date: string }[]>([]);
  ...
  const loadFinancialStats = async () => {
    try {
      ...
      const totalRevenue = completedAppointments.reduce((sum, a) => sum + (a.paymentAmount || 0), 0);
      const pendingPayments = completedAppointments.reduce((sum, a) => sum + (a.remainingBalance || 0), 0);
      const averagePayment = completedAppointments.length > 0 ? totalRevenue / completedAppointments.length : 0;

      // Agrupar pagos por profesional
      const professionalPayments = completedAppointments.reduce((acc, appointment) => {
        const professional = acc.find(p => p.professionalName === appointment.professionalName);
        if (professional) {
        ...
        return acc;
      }, [] as Array<{ professionalName: string; totalAmount: number; pendingAmount: number }>);
  ...
  const loadRecentAbonos = async () => {
    try {
      ...
      const recientes = abonos
        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
        .slice(0, 5)
        .map(a => ({ name: a.professionalName, amount: a.amount, date: a.date }));
  ...
  const generateProfessionalPDF = async (prof: User) => {
    const doc = new jsPDF();
    const now = new Date();
    const fecha = now.toLocaleString('es-ES', { dateStyle: 'long', timeStyle: 'short' });
    const porcentajeIppl = typeof prof.commission === 'number' ? prof.commission : 0;
    ...
    doc.setFontSize(16);
    doc.text('Instituto Psicológico y Psicoanálisis del Litoral', 105, 12, { align: 'center' });
    doc.setFontSize(18);
    ...
    const appointments = await appointmentsService.getProfessionalAppointments(prof.id);
    const patientsWithDebt = patients.map((patient: any) => {
      const debt = appointments
        .filter((a: any) => a.patientId === patient.id && a.status === 'completed' && a.attended)
        .reduce((acc: number, curr: any) => acc + (curr.remainingBalance || 0), 0);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <h3 className="text-2xl font-bold text-gray-900">
                      ${totalSaldoProfesionales.toLocaleString('es-CO', { minimumFractionDigits: 2 })}
                    </h3>
                    <p className="text-sm text-gray-600">Saldo Total Profesionales</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <h3 className="text-2xl font-bold text-gray-900">
                      ${totalDeudaComision.toLocaleString('es-CO', { minimumFractionDigits: 2 })}
                    </h3>
                    <p className="text-sm text-gray-600">Pagos Pendientes con el Instituto</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <h3 className="text-2xl font-bold text-gray-900">
                      ${totalAbonosInstituto.toLocaleString('es-CO', { minimumFractionDigits: 2 })}
                    </h3>
                  ...
                  {recentAbonos.length === 0 ? (
                    ...
                    <div key={idx} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <p className="text-sm font-medium text-gray-900">{abono.name}</p>
                          <p className="text-xs text-gray-500">
                            {new Date(abono.date).toLocaleDateString('es-ES', {
                              year: 'numeric',
                              month: 'long',
                              day: 'numeric'
                            })}
                      ...
                      <div className="text-right">
                        <p className="text-sm font-semibold text-green-600">+${abono.amount.toLocaleString('es-CO', { minimumFractionDigits: 2 })}</p>
      ...
      {selectedProfessional && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-semibold mb-4">Detalle de Profesional</h2>
            <div className="mb-2"><span className="font-bold">Nombre:</span> {selectedProfessional.name}</div>
            <div className="mb-2"><span className="font-bold">Email:</span> {selectedProfessional.email}</div>
            <div className="mb-2"><span className="font-bold">Saldo Total:</span> ${(selectedProfessional.saldoTotal || 0).toFixed(2)}</div>
            <div className="mb-4"><span className="font-bold">Saldo Pendiente:</span> ${(selectedProfessional.saldoPendiente || 0).toFixed(2)}</div>
