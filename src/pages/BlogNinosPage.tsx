...
const BlogNinosPage = () => {
  ...
  const loadPosts = async () => {
    try {
      ...
      const ninosPosts = response.posts.filter((p: Post) => p.section === 'ninos');
      setPosts(ninosPosts);
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 bg-[#F9FAFB] min-h-screen">
      <h1 className="text-3xl font-extrabold mb-8 text-[#374151]">Blog: Niños</h1>
      {posts.length === 0 ? (
