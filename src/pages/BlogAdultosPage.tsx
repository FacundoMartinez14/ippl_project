...
const BlogAdultosPage = () => {
  ...
  const loadPosts = async () => {
    try {
      ...
      const adultosPosts = response.posts.filter((p: Post) => p.section === 'adultos');
      setPosts(adultosPosts);
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 bg-[#F9FAFB] min-h-screen">
      <h1 className="text-3xl font-extrabold mb-8 text-[#374151]">Blog: Adultos</h1>
      {posts.length === 0 ? (
...
export default BlogAdultosPage; 